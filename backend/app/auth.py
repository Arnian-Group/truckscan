...
def require_checklist_agent(current_user: User = Depends(get_current_user)) -> User:
    if not (current_user.is_admin or current_user.can_checklists):
        raise HTTPException(status_code=403, detail="Checklist module access required")
    return current_user
